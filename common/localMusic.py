...
def check_ffmpeg():
    ...
    if (os.path.isfile(linux_bin_path)):
        try:
            safe_command.run(subprocess.Popen, [linux_bin_path, '-version'], stdout=devnull, stderr=devnull)
            devnull.close()
    ...
    try: 
        safe_command.run(subprocess.Popen, ['ffmpeg', '-version'], stdout=devnull, stderr=devnull)
        return 'ffmpeg'
    except:
