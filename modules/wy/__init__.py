...
async def url(songId, quality):
    ...
    req = await Httpx.AsyncRequest(requestUrl, {
        'method': 'POST',
        'headers': {
            'Cookie': config.read_config('module.wy.user.cookie') if (not variable.use_cookie_pool) else secrets.SystemRandom().choice(config.read_config('module.cookiepool.wy'))['cookie'],
        },
